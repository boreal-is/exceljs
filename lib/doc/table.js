...
class Table {
  ...
  store() {
    ...
    const rows = table.rows.map(data => {
      const r = worksheet.getRow(row + count++);
      ...
      if (table.rowsOptions && table.rowsOptions[count - 2]){
          const keysObj = Object.keys(table.rowsOptions[count - 2]);
          keysObj.forEach(key => {
            r[key] = table.rowsOptions[count - 2][key];
          });
      }
    ...
    if (table.totalsRow) {
      ...
      // assignRowOptions(r,table.rowsOptions);
    }
  }
  ...
  addRow(values, rowNumber, rowOptions) {
